...
# Step 1: 创建自定义数据集
# 这个数据集可以保存为 custom_kg.tsv 文件，以下是内容示例：
...
# 使用 pipeline 训练 TransE 模型，并将嵌入维度设置为 1024
...
# Step 3: 提取实体和关系的嵌入
# 1. 提取实体嵌入
entity_embeddings = model.entity_representations[0]  # TransE模型的实体嵌入
entity_embedding_values = entity_embeddings(torch.arange(model.num_entities))

# 2. 提取关系嵌入
relation_embeddings = model.relation_representations[0]  # TransE模型的关系嵌入
...
entity_df.to_csv('./data/entity_embeddings_1024.csv', index=False)
relation_df.to_csv('./data/relation_embeddings_1024.csv', index=False)
